...
function makeAudioEmbed(messageId: string, channelId: string, id: string, filename: string, audioUrl: string, size) {
    ...
    let l = ({ index, content }) => ({
        type: "link",
        target: `https://discord.com/channels/${audioId}/${index}/${channelId}/${messageId}/${id}/${encodedUrl}`,
        content: [{ type: "inlineCode", content }],
    });
...
export default function patch() {
    return after("generate", RowManager.prototype, ([_], { message }: { message }) => {
        ...
        if (audioEmbeds.length) {
            message.embeds.push(...audioEmbeds);
            message.attachments = attachs;
        }
        if (message?.reactions?.length)
            message.reactions = message.reactions.filter((reaction) => reaction.emoji.id != "4259920");
    });
}
