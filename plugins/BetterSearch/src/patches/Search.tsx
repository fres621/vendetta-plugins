...
export default function patch() {
    return after("default", findByName("ConnectedSearchResults", false), (_, searchResultsRes) => {
        after(
            "render",
            searchResultsRes.type.prototype,
            (_, renderRes) => {
                if (!renderRes?.props?.children?.[1].type) return;

                after(
                    "type",
                    renderRes.props.children[1],
                    ([paging], typeRes) => {
                        ...
                        endpoint.props.children =
                            Locale.Messages.PAGINATION_PAGE_OF.format({
                                page,
                            ...
                            <TextInput
                                ...
                                onBlur={() => {
                                    ...
                                    const targetPageOffset =
                                        Math.min(Math.max(page || 1, 1), Math.ceil(paging.totalResults / pageSize)) *
                                        pageSize;
