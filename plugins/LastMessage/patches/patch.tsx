...
addListener((id, timestamp) => {
    let relations = Object.entries(RelationshipStore.getRelationships()).filter(([, type])=>type===1).map(([id])=>id);
    ...
    return (
        <View style={{ marginTop: 10 }}>
            <Text style={[TextStyleSheet['text-xs/medium']]}>
                Last message: {" "}
                { timestamp ? <Timestamp node={{...parseTimestamp(timestamp.toString(), 'R'), type: "timestamp"}} /> 
                : "Unknown"}
            </Text>
        </View>
    );
...
const patch = () => {
    return after("default", find(x=>x?.default?.name==="UserProfileName"), ([props], res) => {
