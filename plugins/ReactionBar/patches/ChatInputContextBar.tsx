...
const Colors = (e)=>{
    const map = {background: "BACKGROUND_SECONDARY", separator: "BACKGROUND_TERTIARY"};
    return map[e] ? resolveSemanticColor(ThemeStore.theme, semanticColors[map[e]]) : "#FFFFFF";
};

type emoji = { totalUses: number, recentUses: string[], frecency: number, score: number };
 ...
 */
let frequentEmojis = sortEmojisByScore(UserSettingsProtoStore.frecencyWithoutFetchingLatest.emojiFrecency.emojis).map(([k, _v]) => k);
...
function toggleReaction(channelId, messageId, reaction) {
    ...
    if (userReactions.find((e)=>e.emoji.name===reaction.name&&e.emoji.id===reaction.id)) { // Remove reaction
        ...
        reactions.addReaction(channelId, messageId, reaction, 'Message', { burst: false });
    };
};
...
export default function() {
    return after("default", findByName("ChatInputContextBar", false), ([a], b)=>{
        ...
        const emojis = savedRES ? savedRES.emojis : frequentEmojis.filter((emoji) => {
          ...
          if (customEmoji.guildId != guildId) {
            if (!PermissionsStore.can(constants.Permissions.USE_EXTERNAL_EMOJIS, GuildStore.getGuild(guildId))) return false; // Server
          ...
          return true;
            ...
            if (s) return {
              uri: 'asset:/emoji-' + Array.from(s).map((c: string) => c.codePointAt(0).toString(16)).join("-") + '.png', 
              name: s, 
              id: null,
              animated: false
            };
        ...
        b.props.children.unshift(
            ...
            style={{ 
                ...
                return (<TouchableOpacity 
                    onPress={ ()=>toggleReaction(channelId, messageId, { id: emoji.id, name: emoji.name, animated: emoji.animated }) }
                    >
                    <Image 
