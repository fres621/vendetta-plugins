    ...
    return (
        <ReactNative.ScrollView style={{ flex: 1 }}>
            <FormSection title="Where to show the top role color?" titleStyleType="no_border">
                <FormSwitchRow
                    label="Show in typing"
                    subLabel="Display the top role color in the typing bar."
                    leading={<FormIcon source={Icons.Typing} />}
                    value={!storage.hideTyping}
                    onValueChange={(v) => (storage.hideTyping = !v)}
                ></FormSwitchRow>

                <FormSwitchRow
                    label="Show in mentions"
                    subLabel="Display the top role color in mentions in the chat."
                    leading={<FormIcon source={Icons.Mention} />}
                    value={!storage.noMention}
                    onValueChange={(v) => (storage.noMention = !v)}
                ></FormSwitchRow>

                <FormSwitchRow
                    label="Show in member list"
                    subLabel="Display the top role color in the member list role section headers."
                    leading={<FormIcon source={Icons.Role} />}
                    value={!storage.noRole}
                    onValueChange={(v) => (storage.noRole = !v)}
                ></FormSwitchRow>

                <View style={{ marginHorizontal: 10, marginBottom: 2 }}>
                    <HelpMessage messageType={0}>
                        (Regarding option below) - Limitations cause text coloring to prevent interaction with the text.
                        Instead, you'll have to press anywhere else in the message to open the context menu or use
                        gestures.
                    </HelpMessage>
                </View>
                <FormSwitchRow
                    label="Show in chat text"
                    subLabel="Display the top role color in the chat text... Why would you want this?"
                    leading={<FormIcon source={Icons.Text} />}
                    value={storage.chatInterpolation > 0}
                    onValueChange={(v) => {
                        storage.chatInterpolation = v ? 100 : 0;
                    }}
                ></FormSwitchRow>
                ...
                <Text
                    style={{
                        marginLeft: "5%",
                        color: interpolateColor(Colors.text, "#ff0000", storage.chatInterpolation / 100),
                ...
                >
                    Color interpolation (for chat text):
                </Text>
                <Swidew
                    onSlide={(v) => {
                        storage.chatInterpolation = v;
                    }}
                    value={storage.chatInterpolation}
                    style={{ marginBottom: 0 }}
                />

                <FormSwitchRow
                    label="Show in voice channel member list"
                    subLabel="Display the top role color in the voice channel member list."
                    leading={<FormIcon source={Icons.Voice} />}
