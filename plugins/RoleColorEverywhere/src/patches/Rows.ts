...
function patchComponents(component, funcs, args, tree?) {
    ...
    if (Array.isArray(component.content)) {
        component.content.forEach(
            (subcomp, index) => (component.content[index] = patchComponents(subcomp, funcs, args, tree)),
        );
    } else if (component.items) {
        component.items.forEach(
            (subcomp, index) => (component.items[index] = patchComponents(subcomp, funcs, args, tree)),
        );
...
function handleRow(row) {
    ...
    const colorPatch = (component, [authorId], tree?) => {
        ...
        if (
            t?.includes("mention") ||
            t?.includes("channelMention") ||
            t?.includes("roleMention") ||
            t?.includes("commandMention") ||
            t?.includes("link")
        )
            return;
            ...
            context: {
                ...
                usernameOnClick: {
                    ...
                    linkColor: ReactNative.processColor(
                        interpolateColor(defaultTextColor, authorMember.colorString, storage.chatInterpolation / 100),
                    ),
                    messageChannelId: "0",
                },
                medium: true,
    ...
    if (message.embeds)
        message.embeds.forEach((embed) => patchComponents({ content: embed.description }, patches, [message.authorId]));
    if (message.referencedMessage?.message?.content)
        patchComponents({ content: message.referencedMessage.message.content }, patches, [
            message.referencedMessage.message.authorId,
        ]);
}
