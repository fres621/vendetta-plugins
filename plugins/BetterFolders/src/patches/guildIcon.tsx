...
export default function () {
    let patch = after("GuildContainer", findByPropsAll("GuildContainer").at(-1), ([props], b) => {
        ...
        if (!GuildFolderPreview?.type) {
            // Makes the plugin work for some older Discord versions
        ...
        let p = after("type", GuildFolderPreview, ([folderProps], g) => {
            ...
            g.props.children[1] = (
                ...
                >
                    <Image
                        source={getAssetIDByName("img_guild_folder")}
                        style={folderProps.folderColor ? { tintColor: `#${folderProps.folderColor.toString(16)}` } : {}}
                    />
                </View>
            );
        });
